...
# Balmorel areas
dict_A = fi[fi['parameter'] == 'balmorel_areas']['value'].iloc[0]
dict_A = dict_A.replace(':', '" : "')
dict_A = dict_A.replace(',', '" , "')
dict_A = dict_A.replace('{', '{"')
dict_A = dict_A.replace('}', '"}')
dict_A = dict_A.replace(' ', '')
...
# dict_A = {conns[i][:3] : AAA[i] for i in range(len(AAA))}
# dict_R = {conns[i].split('_')[0] : AAA[i].split('_')[0] for i in range(len(AAA))}
        ...
        if len(f) > 0:
            ...
            for node in np.unique(f['Node'][node_idx]):
                ...
                if val != 0:
                    ...
                    sign = 1
                string = "GKFX('%s','%s','GNR_EH_HEAT') = GKFX('%s','%s','GNR_EH_HEAT') + %d ;" % (yr, dict_A[node[:3]], yr, dict_A[node[:3]], -val/8736) # CHANGED TO GKFX, check that it's negative!
                fDH = fDH + '\n' + string
            ...
            for node in np.unique(f['Node'][node_idx]):
                ...
                for i in range(len(S)):
                    ...
                    fe.loc[len(hours)*i:(i+1)*len(hours)-1, dict_A[node][:3]] = val # Must be positive
                    # print(fe)
